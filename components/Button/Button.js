...
class Button extends HtmlComp {
	...
	dom() {
		...
		if (this.blob && this.blobFile) { // TODO try to use only one html-Element instead of a and div
			...
			if (!this.btn) this.btn = new Html({parent:this.parent,html:'button',css:css})
	...
	mousedown(evt) {
		const el = Elem.findParent(evt.target,this.div) // use parent may show incorrect when directing direct on <i> Logo
		if (!el) return
		if (!Elem.classStateIs(el,'disabled')) { // will only change something when enabled
			Elem.classStateSet(el,'mousedown',this.mouseState)
		}
	}
	mouseup(evt) {
		const el = Elem.findParent(evt.target,this.div)
		if (!el) return
		Elem.classStateSet(el,'mouseup',this.mouseState)
